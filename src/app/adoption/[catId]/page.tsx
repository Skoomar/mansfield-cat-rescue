...
const Attribute = ({ label, children }: { label: string; children: ReactNode }) => (
    <div className="mb-3">
        <p className="font-medium text-md mb-0.5">{label}</p>
...
const CatPage = async ({ params }: { params: Promise<{ catId: string }> }) => {
    ...
    return (
        <>
            ...
            <main className="py-4 px-6 flex max-lg:flex-col lg:justify-center gap-20 max-w-5xl mx-auto">
                {
                    catInfo.pet.images ?
                        <ImageDisplay images={catInfo.pet.images} /> :
                        <div
                            className="text-center text-gray-700 content-center my-4 w-72 h-80 border rounded border-gray-400 bg-gray-100">No
                            images for this cat!</div>
                }
                <div className="lg:mt-5">
                    <h2 className="max-lg:text-center">{catInfo.pet.name}</h2>
                    <div
                    ...
                    <div className="flex justify-center mt-5">
                        ...
                        <Button asChild
                                className="bg-secondary text-secondary-foreground w-full max-w-72 h-10 hover:bg-accent hover:text-accent-foreground mx-auto shadow-2xl"><Link
